...
| 2026-09-27 | **Phase B batch 9 — CMake link hygiene (commit bf1839d, macos run 36265538683 + arch 36265538657).** Isolated 2-line CMake-only commit: `cc_services` dropped from cc_hooks PUBLIC links and `cc_hooks` from cc_tools PUBLIC links (both dead after the B8 cut). Independent adversarial review built its own parser over 147 TUs (40 hooks + 107 tools: zero reach the removed area), diffed the before/after link closures of every target (only cc_tasks/cc_skills also lost propagated closure — verified zero relevant imports), and forced-touch rebuilt the most-consumed hooks interface (lifecycle_hooks, 11 external importers) + mcp_tool: 80/80 and 92/92 steps clean under `-Werror`, all 34 test exes + loom re-linked — proving no transitive BMI was lost (the clang named-module failure mode). Retained cc_tools→cc_services (27 live service modules until the B15 lift). macos-14 confirms the trim under the Apple scan-deps path: Build 1m57s + Test 2m42s, **1709/1709**; sccache key rotated `fd70d7bf` (CMake inputs), prefix restore **1133/1180 (96.02%), 47 misses, 0 non-cacheable**. Review noted (post-B15 follow-up): cc_skills→cc_tools is already a module-dead link. |
| 2026-09-27 | **Phase B batch 10 (F10-A) — file-access hook extracted into `cc.skills.file_access.port`, confirmed on macos-14 (commit d26e81d, run 36267479299 + arch 36267479232).** The four symbols (FileAccessHook, the function-local storage, set_file_access_hook, notify_file_access incl. try/catch) moved byte-verbatim out of skill.cppm into a cc_skills_core leaf and re-exported; file_read/edit/write now import the rank-5 `.port` contract instead of the concrete skill module (3 edges; is_contract legal). Independent adversarial review confirmed the CENTRAL property with disassembly + nm: the skills registrar (load_skills_dir) and the file tools bind ONE module-attached hook instance/guard in loom and every test binary (single folded weak COMDAT; all static libs). The block move unmasked a genuinely dead `agent_resume -> cc.skills.skill` import (zero token use — logically dead pre-B10), deleted with it; sibling agent skill imports intentionally left for B15. **1709/1709**; file-access notification behavior unchanged. macos-14: Build 2m41s + Test 3m01s, **1709/1709**; sccache **1118/1181 (94.67%), 63 misses, 0 non-cacheable**, key rotated `b83dfbec`. |
